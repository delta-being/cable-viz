...
sys.path.insert(0, str(script_path.parent.parent))  # to find cableviz module
from cv_helper import open_file_append, open_file_read, open_file_write

from cableviz import APP_NAME, __version__, cableviz
...
def build_generated(groupkeys):
    for key in groupkeys:
        ...
        for yaml_file in collect_filenames("Building", key, input_extensions):
            print(f'  "{yaml_file}"')
            cableviz.parse(
                yaml_file, output_formats=("gv", "html", "png", "svg", "tsv")
            )
